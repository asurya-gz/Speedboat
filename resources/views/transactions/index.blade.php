            ...
            <div class="p-6">
                ...
                <div class="mt-4 px-4 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <p class="text-sm text-gray-600 dark:text-gray-300">
                        Total transaksi: <span class="font-medium">{{ $transactions->total() }}</span> 
                        | Halaman: <span class="font-medium">{{ $transactions->currentPage() }}</span> dari <span class="font-medium">{{ $transactions->lastPage() }}</span>
                    </p>
                </div>
                ...
                <div class="mt-6 px-4 py-3 flex items-center justify-between border-t border-gray-200 dark:border-gray-600 sm:px-6">
                    <div class="flex-1 flex justify-between sm:hidden">
                        ...
                        @else
                            <a href="{{ $transactions->appends(['per_page' => $perPage])->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700">
                                Sebelumnya
                            </a>
                        @endif

                        @if($transactions->hasMorePages())
                            <a href="{{ $transactions->appends(['per_page' => $perPage])->nextPageUrl() }}" class="ml-3 relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700">
                                Selanjutnya
                            </a>
                        @else
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        ...
                        <div>
                            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                ...
                                @else
                                    <a href="{{ $transactions->appends(['per_page' => $perPage])->previousPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm font-medium text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                        </svg>
                                    </a>
                                ...
                                {{-- Pagination Elements --}}
                                @foreach($transactions->appends(['per_page' => $perPage])->getUrlRange(1, $transactions->lastPage()) as $page => $url)
                                ...
                                {{-- Next Page Link --}}
                                @if($transactions->hasMorePages())
                                    <a href="{{ $transactions->appends(['per_page' => $perPage])->nextPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm font-medium text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
