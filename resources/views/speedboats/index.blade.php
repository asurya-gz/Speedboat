...
@section('title', 'Daftar Speedboat')

@section('header-actions')
    <div class="flex space-x-3">
        @if(!Auth::user()->isKasir())
            <a href="{{ route('speedboats.create') }}" class="btn btn-primary">
...
<div class="max-w-7xl mx-auto">
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
            <form method="GET" action="{{ route('speedboats.index') }}" class="flex flex-wrap gap-4 items-end">
                <!-- Search Input -->
                <div class="flex-1 min-w-64">
                    ...
                    <div class="relative">
                        <input type="text" 
                               ...
                               placeholder="Cari berdasarkan kode, nama, tipe, atau deskripsi..." 
                               class="block w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 dark:text-white dark:placeholder-gray-400 text-sm">
                ...
                <div class="min-w-48">
                    ...
                    <select id="capacity" 
                        ...
                        <option value="small" {{ request('capacity') === 'small' ? 'selected' : '' }}>Kecil (1-20 orang)</option>
                        <option value="medium" {{ request('capacity') === 'medium' ? 'selected' : '' }}>Sedang (21-50 orang)</option>
                        <option value="large" {{ request('capacity') === 'large' ? 'selected' : '' }}>Besar (51+ orang)</option>
                    </select>
                </div>
                ...
                <div class="flex space-x-2">
                    ...
                    <a href="{{ route('speedboats.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white text-sm font-medium rounded-lg transition-colors duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-6">
            @if($speedboats->total() > 0)
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Nama Speedboat</th>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @if($speedboat->description)
                                        <div class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ Str::limit($speedboat->description, 50) }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @if(Auth::user()->isAdmin())
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            <a href="{{ route('speedboats.show', $speedboat) }}" 
                                            ...
                                            <a href="{{ route('speedboats.edit', $speedboat) }}" 
                                            ...
                                            <form action="{{ route('speedboats.toggle-status', $speedboat) }}" 
                                                  method="POST" 
                                                  class="inline-block">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" 
                                                        ...
                                                        title="{{ $speedboat->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                                    @if($speedboat->is_active)
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-4 px-4 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <p class="text-sm text-gray-600 dark:text-gray-300">
                        Total speedboat: <span class="font-medium">{{ $speedboats->total() }}</span> 
                        | Halaman: <span class="font-medium">{{ $speedboats->currentPage() }}</span> dari <span class="font-medium">{{ $speedboats->lastPage() }}</span>
                    </p>
                </div>
                ...
                @if($speedboats->total() > 0)
                    <div class="mt-6 px-4 py-3 flex items-center justify-between border-t border-gray-200 dark:border-gray-600 sm:px-6">
                        <div class="flex-1 flex justify-between sm:hidden">
                            ...
                            @else
                                <a href="{{ $speedboats->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700">
                                    Sebelumnya
                                </a>
                            @endif

                            @if($speedboats->hasMorePages())
                                <a href="{{ $speedboats->nextPageUrl() }}" class="ml-3 relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700">
                                    Selanjutnya
                                </a>
                            @else
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700 dark:text-gray-300">
                                    Menampilkan
                                    <span class="font-medium">{{ $speedboats->firstItem() }}</span>
                                    sampai
                                    <span class="font-medium">{{ $speedboats->lastItem() }}</span>
                                    dari
                                    <span class="font-medium">{{ $speedboats->total() }}</span>
                                    speedboat
                                </p>
                            </div>
                            <div>
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                    ...
                                    @else
                                        <a href="{{ $speedboats->previousPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm font-medium text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                                            <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                            </svg>
                                        </a>
                                    ...
                                    {{-- Pagination Elements --}}
                                    @foreach($speedboats->getUrlRange(1, $speedboats->lastPage()) as $page => $url)
                                        @if($page == $speedboats->currentPage())
                                    ...
                                    {{-- Next Page Link --}}
                                    @if($speedboats->hasMorePages())
                                        <a href="{{ $speedboats->nextPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm font-medium text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                                            <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
            ...
            @else
                <div class="text-center py-16">
                    ...
                    <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">Belum ada speedboat</h3>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        @if(Auth::user()->isKasir())
                            Hubungi administrator untuk menambahkan speedboat
                        @else
                            Tambahkan speedboat pertama Anda
                        @endif
                    </p>
                    @if(!Auth::user()->isKasir())
                        <div class="mt-6">
